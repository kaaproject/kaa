...
/**
 * Default implementation of {@link DeltaCalculator}.
 *
 * @author Yaroslav Zeygerman
 */
public class DefaultDeltaCalculator implements DeltaCalculator {

    /** The Constant logger. */
    private static final Logger LOG = LoggerFactory //NOSONAR
            .getLogger(DefaultDeltaCalculator.class);

    /** The Constant SYSTEM_NAMESPACE. */
    private static final String SYSTEM_NAMESPACE = "org.kaaproject.configuration";
    
    /** The Constant UUID. */
    private static final String UUID = "uuid";

    /** The Constant UUID_FIELD. */
    private static final String UUID_FIELD = "__" + UUID;
    ...
    private class FieldAttribute {
         ...
         */
        private DefaultDeltaCalculator getOuterType() {
            return DefaultDeltaCalculator.this;
        }
     ...
     */
    public DefaultDeltaCalculator(Schema deltaSchema, Schema baseSchema) {
    ...
    private static void putUnchanged(GenericRecord delta, String field) throws DeltaCalculatorException {
        Schema unchangedSchema = getSchemaByFullName(delta, field, SYSTEM_NAMESPACE + ".unchangedT");
    ...
    private static void putReset(GenericRecord delta, String field) throws DeltaCalculatorException {
        Schema resetSchema = getSchemaByFullName(delta, field, SYSTEM_NAMESPACE + ".resetT");
        if (resetSchema != null) {
    ...
    /* (non-Javadoc)
     * @see org.kaaproject.kaa.server.operations.service.delta.DeltaCalculator#calculate(org.kaaproject.kaa.common.dto.EndpointConfigurationDto, org.kaaproject.kaa.common.dto.ConfigurationDto)
     */
    @Override
    public RawBinaryDelta calculate(String endpointConfiguration, String newConfigurationBody)
    ...
    /* (non-Javadoc)
     * @see org.kaaproject.kaa.server.operations.service.delta.DeltaCalculator#calculate(org.kaaproject.kaa.common.dto.ConfigurationDto)
     */
    @Override
    public RawBinaryDelta calculate(String newConfigurationBody) throws IOException, DeltaCalculatorException {
        GenericRecord newRoot = getRootNode(newConfigurationBody, baseSchema);
        return calculate(newRoot);
    }

    @Override
    public RawBinaryDelta calculate(byte [] endpointConfiguration, byte [] newConfigurationBody)
            throws IOException, DeltaCalculatorException {
        GenericRecord oldRoot = getRootNode(endpointConfiguration, baseSchema);
        GenericRecord newRoot = getRootNode(newConfigurationBody, baseSchema);
        return calculate(oldRoot, newRoot);
    ...
    @Override
    public RawBinaryDelta calculate(byte [] newConfigurationBody) throws IOException, DeltaCalculatorException {
        GenericRecord newRoot = getRootNode(newConfigurationBody, baseSchema);
        return calculate(newRoot);
    }
     ...
     */
    private GenericRecord getRootNode(String json, Schema schema) throws IOException {
        GenericAvroConverter<GenericRecord> converter = new GenericAvroConverter<GenericRecord>(schema);
        return converter.decodeJson(json);
    }
     ...
     * @return the root node
     */
    private GenericRecord getRootNode(byte [] data, Schema schema) throws IOException {
        GenericAvroConverter<GenericRecord> converter = new GenericAvroConverter<GenericRecord>(schema);
        return converter.decodeJson(data);
    }
}
