...
public class DefaultNotificationDeltaService implements NotificationDeltaService {
    ...
    public GetNotificationResponse getNotificationDelta(GetNotificationRequest request, HistoryDelta historyDelta) {
        ...
        if (historyDelta.isTopicListChanged()) {
            ...
            // TODO: validate change based on topic list hash that is stored in profile
        ...
        for (String topicId : subscriptionSet) {
            ...
            LOG.debug("[{}] fetch new subscriptions for topic id: {}, system schema version {}, user schema version {}, starting seq number {}", endpointId, topicId,
            		profile.getSystemNfVersion(), profile.getUserNfVersion(), seqNumber);
            List<NotificationDto> topicNotifications = notificationService.findNotificationsByTopicIdAndVersionAndStartSecNum(topicId, seqNumber,
                    profile.getSystemNfVersion(), profile.getUserNfVersion());
    ...
    private Set<String> buildSubscriptionSet(EndpointProfileDto profile) {
        ...
        List<String> subscriptionList = profile.getSubscriptions();
        if (subscriptionList != null) {
            subscriptionSet = new HashSet<String>(profile.getSubscriptions());
        } else {
    ...
    /* (non-Javadoc)
     * @see org.kaaproject.kaa.server.operations.service.notification.NotificationDeltaService#findNotificationById(java.lang.String)
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see org.kaaproject.kaa.server.operations.service.notification.NotificationDeltaService#findUnicastNotificationById(java.lang.String)
     */
    @Override
