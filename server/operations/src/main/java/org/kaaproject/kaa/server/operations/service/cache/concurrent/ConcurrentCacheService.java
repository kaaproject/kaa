...
public class ConcurrentCacheService implements CacheService {
    ...
    private final CacheTemporaryMemorizer<String, Integer> appSeqNumberMemorizer = new CacheTemporaryMemorizer<String, Integer>();
    ...
    private final CacheTemporaryMemorizer<List<EndpointGroupStateDto>, byte[]> mergedConfigurationMemorizer = new CacheTemporaryMemorizer();
    ...
    public int getAppSeqNumber(String key) {
        return appSeqNumberMemorizer.compute(key, new Computable<String, Integer>() {
            @Override
            public Integer compute(String key) {
                LOG.debug("Fetching result for getAppSeqNumber");
                Integer value = applicationService.findAppByApplicationToken(key).getSequenceNumber();
                putAppSeqNumber(key, value);
                return value;
            }
        });
    }
    ...
    /* (non-Javadoc)
     * @see org.kaaproject.kaa.server.operations.service.cache.CacheService#getMergedConfiguration(java.util.List, org.kaaproject.kaa.server.operations.service.cache.Computable)
     */
    @Override
    @Cacheable(value = "mergedConfigurations", key = "#key")
    public byte[] getMergedConfiguration(final List<EndpointGroupStateDto> key, final Computable<List<EndpointGroupStateDto>, byte[]> worker) {
        return mergedConfigurationMemorizer.compute(key, new Computable<List<EndpointGroupStateDto>, byte[]>() {

            @Override
            public byte[] compute(List<EndpointGroupStateDto> key) {
                LOG.debug("Fetching result for getMergedConfiguration");
                byte[] result = worker.compute(key);
    ...
    /* (non-Javadoc)
     * @see org.kaaproject.kaa.server.operations.service.cache.CacheService#setMergedConfiguration(java.util.List, java.lang.String)
     */
    @Override
    @CachePut(value = "mergedConfigurations", key = "#key")
    public byte[] setMergedConfiguration(List<EndpointGroupStateDto> key, byte[] mergedConfiguration) {
    ...
    public DeltaCacheEntry getDelta(final DeltaCacheKey key, final Computable<DeltaCacheKey, DeltaCacheEntry> worker) throws GetDeltaException {
        return deltaMemorizer.compute(key, new Computable<DeltaCacheKey, DeltaCacheEntry>() {
            @Override
