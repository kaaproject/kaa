...
import org.kaaproject.kaa.server.operations.service.delta.AvroBinaryDelta;
...
public class DefaultDeltaCalculatorTest {
    ...
    public static final String OLD_COMPLEX_CONFIG = "service/delta/complexFieldsDeltaCurrent.json";
    public static final String COMPLEX_SCHEMA = "service/delta/complexFieldsDeltaSchema.json";
    public static final String COMPLEX_PROTOCOL_SCHEMA = "service/delta/complexFieldsDeltaProtocolSchema.json";
    ...
    public void testPrimitiveFieldsDelta() throws IOException, DeltaCalculatorException {
        URL protocolSchemaUrl = Thread.currentThread().getContextClassLoader().getResource("service/delta/primitiveFieldsDeltaProtocolSchema.json");
        Schema protocolSchema = new Schema.Parser().parse(new File(protocolSchemaUrl.getPath()));
        URL schemaUrl = Thread.currentThread().getContextClassLoader().getResource("service/delta/primitiveFieldsDeltaSchema.json");
        ...
        URL oldConfigUrl = Thread.currentThread().getContextClassLoader().getResource("service/delta/primitiveFieldsDeltaCurrent.json");
        ...
        URL newConfigUrl = Thread.currentThread().getContextClassLoader().getResource("service/delta/primitiveFieldsDeltaNew.json");
    ...
    public void testArrayPrimitiveFieldsDelta() throws IOException, URISyntaxException, DeltaCalculatorException {
        URL protocolSchemaUrl = Thread.currentThread().getContextClassLoader().getResource("service/delta/arrayPrimitiveFieldsDeltaProtocolSchema.json");
        Schema protocolSchema = new Schema.Parser().parse(new File(protocolSchemaUrl.getPath()));
        URL schemaUrl = Thread.currentThread().getContextClassLoader().getResource("service/delta/arrayPrimitiveFieldsDeltaSchema.json");
        Schema schema = new Schema.Parser().parse(new File(schemaUrl.getPath()));
        DeltaCalculator calculator = new DefaultDeltaCalculator(protocolSchema, schema);

        ByteArrayOutputStream baosOld = new ByteArrayOutputStream();
        URL oldConfigUrl = Thread.currentThread().getContextClassLoader().getResource("service/delta/arrayPrimitiveFieldsDeltaCurrent.json");
        IOUtils.copy(new FileInputStream(oldConfigUrl.getPath()), baosOld, 1024);
        String oldStr = new String(baosOld.toByteArray(), "UTF-8");

        ConfigurationDto newConfig = new ConfigurationDto();
        ByteArrayOutputStream baosNew = new ByteArrayOutputStream();
        URL newConfigUrl = Thread.currentThread().getContextClassLoader().getResource("service/delta/arrayPrimitiveFieldsDeltaNew.json");
        IOUtils.copy(new FileInputStream(newConfigUrl.getPath()), baosNew, 1024);
        String newStr = new String(baosNew.toByteArray(), "UTF-8");
        newConfig.setBinaryBody(newStr.getBytes());
