...
public class EndpointProfileMongoDaoTest extends AbstractTest {
    ...
    public void removeByIdTest() {
        EndpointProfile endpointProfile = endpointProfileDao.findById(endProfiles.get(0));
        Assert.assertNotNull(endpointProfile);
        endpointProfileDao.removeById(endpointProfile.getId());
        endpointProfile = endpointProfileDao.findById(endpointProfile.getId());
        Assert.assertNull(endpointProfile);
    }
    ...
    public void saveEndpointProfileTest() {
        ...
        EndpointProfile saved = endpointProfileDao.save(endpointProfile);
        Assert.assertNotNull(saved);
        Assert.assertEquals(endpointProfile, saved);
    }
    ...
    public void convertToDtoTest() {
        EndpointProfile endpointProfile = endpointProfileDao.findById(endProfiles.get(0));
        Assert.assertNotNull(endpointProfile);
        EndpointProfileDto dto = endpointProfile.toDto();
        EndpointProfile converted = new EndpointProfile(dto);
        Assert.assertEquals(endpointProfile, converted);
    }
}
