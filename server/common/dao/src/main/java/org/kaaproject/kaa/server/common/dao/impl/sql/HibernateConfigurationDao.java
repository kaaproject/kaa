...
@Repository
public class HibernateConfigurationDao extends HibernateAbstractDao<Configuration>
    implements ConfigurationDao<Configuration> {
  ...
  public Configuration findConfigurationByAppIdAndVersion(String appId, int version) {
    ...
    if (isNotBlank(appId)) {
      configuration = findOneByCriterionWithAlias(
  ...
  public Configuration findConfigurationByEndpointGroupIdAndVersion(String groupId, int version) {
    Configuration configuration = null;
    LOG.debug("Searching configuration by endpoint group id [{}] and schema version [{}]",
        groupId, version);
    if (isNotBlank(groupId)) {
      configuration = findOneByCriterionWithAlias(
          ENDPOINT_GROUP_PROPERTY,
          ENDPOINT_GROUP_ALIAS,
          Restrictions.and(
  ...
  public List<Configuration> findActiveByEndpointGroupId(String groupId) {
    ...
    if (isNotBlank(groupId)) {
      configurations = findListByCriterionWithAlias(
          ENDPOINT_GROUP_PROPERTY,
          ENDPOINT_GROUP_ALIAS,
          Restrictions.and(
              Restrictions.eq(ENDPOINT_GROUP_REFERENCE, Long.valueOf(groupId)),
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace("[{}] Search result: {}.",
          groupId, Arrays.toString(configurations.toArray()));
    } else {
      LOG.debug("[{}] Search result: {}.",
          groupId, configurations.size());
    }
  ...
  public List<Configuration> findActualBySchemaIdAndGroupId(String schemaId, String groupId) {
    ...
    LOG.debug("Searching actual configurations by configuration schema id [{}] "
            + "and group id [{}] ",
        schemaId, groupId);
    if (isNotBlank(schemaId) && isNotBlank(groupId)) {
      Criteria criteria = getCriteria();
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace("[{},{}] Search result: {}.",
          schemaId, groupId, Arrays.toString(configurations.toArray()));
    } else {
      LOG.debug("[{},{}] Search result: {}.",
          schemaId, groupId, configurations.size());
    }
    return configurations;
  }
  ...
  public void removeByConfigurationSchemaId(String schemaId) {
    if (isNotBlank(schemaId)) {
      List<Configuration> configurations = findListByCriterionWithAlias(
          CONFIGURATION_SCHEMA_PROPERTY,
  ...
  public Configuration save(Configuration object) {
    Configuration saved = super.save(object);
    getSession().flush();
    return saved;
  }
