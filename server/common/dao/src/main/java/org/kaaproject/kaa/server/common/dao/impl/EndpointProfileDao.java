...
public interface EndpointProfileDao<T extends EndpointProfile> extends Dao<T, ByteBuffer> {
   ...
   * @return <code>true</code> if there is at least one endpoint profile with the given SDK token,
   * <code>false</code> otherwise
   */
  boolean checkSdkToken(String sdkToken);

  /**
   * Update endpoint profile with given keyHash, server profile schema version and given server
   * profile.
   *
