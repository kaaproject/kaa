...
public class EndpointNotificationCassandraDao extends AbstractCassandraDao<CassandraEndpointNotification, String> implements EndpointNotificationDao<CassandraEndpointNotification> {
    ...
    protected String getColumnFamilyName() {
        return CassandraModelConstants.ET_NF_COLUMN_FAMILY_NAME;
    }
    ...
    public List<CassandraEndpointNotification> findNotificationsByKeyHash(byte[] keyHash) {
        ...
        if (keyHash != null) {
            Select.Where where = select().from(getColumnFamilyName()).where(eq(CassandraModelConstants.ET_NF_ENDPOINT_KEY_HASH_PROPERTY, getByteBuffer(keyHash)));
    ...
    public void removeNotificationsByKeyHash(byte[] keyHash) {
        ...
        execute(delete().from(getColumnFamilyName()).where(eq(CassandraModelConstants.ET_NF_ENDPOINT_KEY_HASH_PROPERTY, getByteBuffer(keyHash))));
    }
    ...
    public CassandraEndpointNotification findById(String id) {
        ...
        Select.Where where = select().from(getColumnFamilyName()).where(eq(CassandraModelConstants.ET_NF_ENDPOINT_KEY_HASH_PROPERTY, key.getEndpointKeyHash()))
    ...
    public void removeById(String id) {
        ...
        CassandraEndpointNotification key = new CassandraEndpointNotification(id);
        Statement delete = delete().from(getColumnFamilyName()).where(eq(CassandraModelConstants.ET_NF_ENDPOINT_KEY_HASH_PROPERTY, key.getEndpointKeyHash()))
                .and(eq(CassandraModelConstants.ET_NF_LAST_MOD_TIME_PROPERTY, key.getLastModifyTime()));
        execute(delete);
