...
public class EndpointUserCassandraDao
    extends AbstractVersionableCassandraDao<CassandraEndpointUser, String>
    implements EndpointUserDao<CassandraEndpointUser> {
  ...
  @Override
  public CassandraEndpointUser save(CassandraEndpointUser object) {
    if (isBlank(object.getId())) {
      object.generateId();
    }
    LOG.trace("Save endpoint user {}", object);
  ...
  public void removeByExternalIdAndTenantId(String externalId, String tenantId) {
    LOG.debug("Try to remove endpoint user by external id {} and tenant id {}",
        externalId, tenantId);
  ...
  public boolean checkAccessToken(String externalId, String tenantId, String accessToken) {
    LOG.debug("Check access token [{}] for endpoint user with external id {} and tenant id {}",
        accessToken, externalId, tenantId);
