...
public class CassandraLogEventDao implements LogEventDao {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(CassandraLogEventDao.class);
	...
	public CassandraLogEventDao(CassandraConfig configuration)
		...
		if (configuration != null) {
			...
			for (CassandraServer node : nodes) {
				clusterNodes.add(new InetSocketAddress(InetAddress
						.getByName(node.getHost()), node.getPort()));
			}

			Cluster.Builder builder = Cluster.builder()
					.addContactPointsWithPorts(clusterNodes);
			LOG.info("Init cassandra cluster with nodes {}",
					Arrays.toString(clusterNodes.toArray()));

			CassandraCredential cc = configuration.getCassandraCredential();
			if (cc != null) {
				builder.withCredentials(cc.getUser(), cc.getPassword());
				LOG.trace(
						"Init cassandra cluster with username {} and password {}",
						cc.getUser(), cc.getPassword());
			}

			CassandraSocketOption option = configuration
					.getCassandraSocketOption();
			if (option != null) {
				...
				LOG.trace("Init cassandra cluster with socket options {}",
						option);
			}

			CassandraWriteConsistencyLevel ccLevel = configuration
					.getCassandraWriteConsistencyLevel();
			if (ccLevel != null) {
				writeConsistencyLevel = ConsistencyLevel
						.valueOf(ccLevel.name());
				LOG.trace("Init cassandra cluster with consistency level {}",
						ccLevel.name());
			}
			CassandraCompression cassandraCompression = configuration
					.getCassandraCompression();
			if (cassandraCompression != null) {
				builder.withCompression(ProtocolOptions.Compression
						.valueOf(cassandraCompression.name()));
				LOG.trace("Init cassandra cluster with compression {}",
						cassandraCompression.name());
			}
	...
	public String createTable(String appToken) {
		...
		LOG.info("Create table {} in cassandra keyspace {}", tableName,
				keyspaceName);
		...
		createTableStmt = createTableStmt.replace(COLUMNS_DEFINITION,
				generateColumnsDefinition(configuration));
		createTableStmt = createTableStmt.replace(PRIMARY_KEY_DEFINITION,
				generatePrimaryKeyDefinition(configuration));
		createTableStmt = createTableStmt.replace(CLUSTERING_DEFINITION,
				generateClusteringDefinition(configuration));
	...
	private String toFullTableName(String appToken) {
		...
		tableName = tableName.replace($CONFIG_HASH,
				Integer.toString(Math.abs(configuration.hashCode())));
		return tableName;
	}

	private static String generateColumnsDefinition(
			CassandraConfig configuration) {
	...
	private static String generatePrimaryKeyDefinition(
			CassandraConfig configuration) {
		List<ColumnMappingElement> mappings = configuration.getColumnMapping();
	...
	private static String generateClusteringDefinition(
			CassandraConfig configuration) {
	...
	@Override
	public List<CassandraLogEventDto> save(
			List<CassandraLogEventDto> logEventDtoList, String tableName,
		...
		executeBatch(prepareQuery(logEventDtoList, tableName, eventConverter,
				headerConverter));
		return logEventDtoList;
	}

	@Override
	public ListenableFuture<ResultSet> saveAsync(
			List<CassandraLogEventDto> logEventDtoList, String tableName,
			GenericAvroConverter<GenericRecord> eventConverter,
			GenericAvroConverter<GenericRecord> headerConverter)
			throws IOException {
		LOG.debug("Execute async bath request for cassandra table {}",
				tableName);
	...
	private Insert[] prepareQuery(List<CassandraLogEventDto> logEventDtoList,
		...
		for (int i = 0; i < logEventDtoList.size(); i++) {
			...
			Insert insert = QueryBuilder.insertInto(keyspaceName,
					collectionName);
			for (ColumnMappingElement element : configuration
					.getColumnMapping()) {
				switch (element.getType()) {
				case HEADER_FIELD:
					insert.value(element.getColumnName(),
							dto.getHeader().get(element.getValue()));
					break;
				case EVENT_FIELD:
					insert.value(element.getColumnName(),
							dto.getEvent().get(element.getValue()));
					break;
				case HEADER_JSON:
					insert.value(element.getColumnName(),
					...
					break;
				case HEADER_BINARY:
					insert.value(element.getColumnName(), ByteBuffer
							.wrap(headerConverter.encode(dto.getHeader())));
					break;
				case EVENT_JSON:
					insert.value(element.getColumnName(),
				...
				case EVENT_BINARY:
					insert.value(element.getColumnName(), ByteBuffer
							.wrap(eventConverter.encode(dto.getEvent())));
					break;
				case UUID:
