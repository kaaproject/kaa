...
public class HBaseLogEventDao implements LogEventDao{
	...
	public HBaseLogEventDao(HBaseAppenderConfiguration config){
		...
		String zk_port = configuration.getServer().getPort().toString();
		
		LOG.info("Connecting to Zookeeper Quorum -   "+zk+":"+zk_port);
		
		try{
			//Create HBase configuration
    		Configuration hbconf = HBaseConfiguration.create();
    		hbconf.set("hbase.zookeeper.quorum",zk);
    		hbconf.set("hbase.zookeeper.property.clientPort", zk_port);
	 ...
	 */
	private void addColumnFamilies(List<ColumnFamily> columnFamilies, HTableDescriptor tableDescriptor) {
		// TODO Auto-generated method stub
		for(ColumnFamily columnf: columnFamilies) {
			HColumnDescriptor cf = new HColumnDescriptor(columnf.getCfName().toString().toLowerCase().trim());
	...
	private Algorithm GetAlgorithmCompression(org.kaaproject.kaa.server.appenders.hbase.config.gen.Algorithm compression) {
		// TODO Auto-generated method stub
		switch(compression) {
			case GZ:
				return Algorithm.GZ;
	...
	public void save(List<HBaseLogEventDto> logEventDtoList, String collectionName,
		...
		for (int i = 0; i < logEventDtoList.size(); i++) {
			...
			for (ColumnMappingElement element : rowKeyMap) {
				String row_element=stringConverter(element.getValueType(), dto.getEvent().get(element.getValue().toString()));
