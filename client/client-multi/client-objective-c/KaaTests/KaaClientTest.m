...
- (instancetype)initWithPlatformContext:(id<KaaClientPlatformContext>)context delegate:(id<KaaClientStateDelegate>)delegate bootstrapManager:(id<BootstrapManager>)bsManager {
    self = [self initWithPlatformContext:context delegate:delegate autoGeneratedKeys:YES];
...
- (void)testDefaultKeyGenerationStrategy {
    [KeyUtils deleteExistingKeyPair];
    
    @try {
        self.client = [KaaClientFactory clientWithContext:self.context stateDelegate:self.delegate];
        XCTFail(@"testDefaultKeyGenerationStrategy failed");
    } @catch (NSException *exception) {
        NSLog(@"testDefaultKeyGenerationStrategy succeeded");
    }
}

- (void)testUserKeyGenerationStrategy {
    [KeyUtils deleteExistingKeyPair];
    
    self.client = [KaaClientFactory clientWithContext:self.context stateDelegate:self.delegate autoGeneratedKeys:YES];
}
