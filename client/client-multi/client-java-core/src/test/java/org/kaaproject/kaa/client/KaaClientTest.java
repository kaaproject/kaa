...
public class KaaClientTest {
    ...
    protected void initStorageMock(PersistentStorage storage) throws NoSuchAlgorithmException, IOException {
        ...
        Mockito.when(storage.openForRead(KaaClientPropertiesState.CLIENT_PUBLIC_KEY_NAME_DEFAULT)).thenReturn(
                new ByteArrayInputStream(kp.getPublic().getEncoded()));
        Mockito.when(storage.openForRead(KaaClientPropertiesState.CLIENT_PRIVATE_KEY_NAME_DEFAULT)).thenReturn(
