...
:buildAvro
  ...
  if %BUILD_PLATFORM% == x86 (
    cmake -DCMAKE_INSTALL_PREFIX:PATH=%AVRO_ROOT_DIR% -G "Visual Studio %MSVC_VERSION%"  ..
  ) else (
    cmake -DCMAKE_INSTALL_PREFIX:PATH=%AVRO_ROOT_DIR% -G "Visual Studio %MSVC_VERSION% Win64"  ..
  )
...
:buildBotan
  ...
  if %BUILD_TYPE%==debug (
    python configure.py --cc=msvc --cpu=%ARCH% --prefix=%BOTAN_HOME% --with-debug-info --no-optimizations
  ) else (
    python configure.py --cc=msvc --cpu=%ARCH% --prefix=%BOTAN_HOME%
  ) 
