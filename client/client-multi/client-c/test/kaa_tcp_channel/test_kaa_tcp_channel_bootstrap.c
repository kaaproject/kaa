...
static uint8_t KAASYNC_BOOTSTRAP[] = {
    0xF0, 0x13, 0x00, 0x06, 'K', 'a','a','t','c','p',
    0x01, 0x00, 0x01, 0x10, 'K', 'a','a','t','c','p', 0x00
};

static const uint8_t *KAASYNC_BOOTSTRAP_MESSAGE = (const uint8_t *)"Kaatcp";
...
void test_create_kaa_tcp_channel(void **state)
{
    ...
    ASSERT_EQUAL(r_supported_services[0], KAA_EXTENSION_BOOTSTRAP);
...
{
    (void)self;
    // TODO(rasen): oh-oh. we must use buffer_size here
    (void)buffer_size;

    KAA_RETURN_IF_NIL(buffer, KAA_ERR_BADPARAM);

    if (!memcmp(buffer, KAASYNC_BOOTSTRAP_MESSAGE, strlen((const char *)KAASYNC_BOOTSTRAP_MESSAGE))) {
