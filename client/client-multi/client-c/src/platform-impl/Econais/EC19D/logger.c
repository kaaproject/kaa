...
typedef enum {DEFAULT,PERCENT_FIRST_FOUND,PERCENT_FOUND} sprintfState;
typedef char* char_p;
typedef void* void_p;
...
//	va_start(args, format);
	...
	for (i=0; i<format_length;i++) {
		...
		p++;
		if (no_more_space_in_buffer) break;
	}
//	sndc_printf("p=0x%06x, last_format=0x%06x, buffer_size_l=%u, (p - last_format)=%u\n",p,last_format,buffer_size_l,(p - last_format));
...
//	sndc_printf("res_len=%u\n",res_len);
//	int res_len = snprintf(buffer
//					, buffer_size
//					, format
//					, args);
