...
kaa_error_t kaa_user_compile_request(kaa_user_manager_t *self, kaa_user_sync_request_t** request_p, size_t requestId)
{
    ...
    *request_p = NULL;
    ...
    if (self->user_info && !self->is_waiting_user_attach_response) {
        ...
        user_attach_request->user_external_id = self->user_info->user_external_id; // destructor is not needed
        user_attach_request->user_access_token = self->user_info->user_access_token; // destructor is not needed
