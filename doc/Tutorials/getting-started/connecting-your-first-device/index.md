...
Let's start from basic terms definition.
...
When some message from the device arrives to the Kaa platform, the device-related endpoint token is resolved into the corresponding endpoint ID.
...
Speaking of the endpoint metadata format, an endpoint metadata is a JSON document of an arbitrary format that is associated with a specific endpoint.


## Playbook

Let's connect a simulator, retrieve and report its metadata attributes from/to the Kaa platform.
...
**2**. Open the [client.py](https://repl.it/@KaaIoT/ConnectDeviceToIotPlatform) simulator script, specify the endpoint token from the previous step as well as the application version from the device page, and run the script.
...
Swap `<app-version-name>` with application version name and `<endpoint-token>` with endpoint token.
