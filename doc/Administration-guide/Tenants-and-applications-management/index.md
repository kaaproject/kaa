...
To export the system CT, click **Export** and select export method from drop-down. See [CT schema export]({{root_url}}Programming-guide/Key-platform-features/Common-Type-Library/#ct-schema-export) for available export methods details.
...
To use the Kaa [events]({{root_url}}Programming-guide/Key-platform-features/Events/) for one or more applications, the tenant admin should create an event class family (ECF). Each ECF should be described using the [Avro](http://avro.apache.org/docs/current/spec.html) format.
...
To export the CT, click Export and select export method from drop-down.
...
To export the server-side EP profile schema, click **Export** in the last column of desired schema row and select export method from drop-down.
...
To start using this tool, [install and run Kaa Sandbox]({{root_url}}Getting-started/) at first and then go to the following URL (by default): [http://127.0.0.1:9080/avroUiSandbox/](http://127.0.0.1:9080/avroUiSandbox/).
